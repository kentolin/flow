  ...
  let clipboard = { nodes: [], edges: [] };
    ...
    // ==========================================================
      ...
      if ((e.key === "Delete" || e.key === "Backspace")) {
        ...
        if (nodeManager.selectedNode) selectedNodeIds.add(nodeManager.selectedNode);
    ...
    if (e.ctrlKey && !e.shiftKey && e.key.toLowerCase() === "a") {
      ...
      // 🔔 Visual feedback
      showToast(`Selected ${state.nodes.length} nodes and ${state.edges.length} edges`, "info");
...
function showToast(message, type = null) {
  ...
  if (!type) {
    ...
    else if (lowerMsg.includes("delete") || lowerMsg.includes("removed")) type = "error";
    else if (lowerMsg.includes("copy") || lowerMsg.includes("paste") || lowerMsg.includes("saved")) type = "success";
    else if (lowerMsg.includes("clear") || lowerMsg.includes("empty")) type = "warn";
    else if (lowerMsg.includes("select") || lowerMsg.includes("highlight")) type = "info";
    else type = "info";
  }
  ...
  window.dispatchEvent(new CustomEvent("toast:shown", { detail: { message, type } }));
